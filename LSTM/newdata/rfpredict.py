...
def get_selcet_clean_data(data, threshold, c):
    data1 = pd.read_csv('./select_data/' + data + '_' + threshold + '_' + str(select_hour[c][0]) + '.csv', encoding='utf-8')
...
def get_other_data(data, threshold, c):
    data1 = pd.read_csv('./other_data/' + data + '_' + threshold + '_' + str(select_hour[c][0]) + '_other.csv',
                        encoding='utf-8')
...
for t in traindata:
    for th in Threshold_Folder:
        ...
        for dc in data_container:
            ...
            RF_Model = RandomForestRegressor(n_estimators=300, max_features=1, oob_score=True)
            labels = train_Y
            features = train_X

            rgr = RF_Model.fit(features, labels)

            X_test_predict = rgr.predict(test_X)

            predicted_yb_count = X_test_predict * yb_data.max(axis=0)
            test_Y = test_Y * yb_data.max(axis=0)
            plt.plot(test_Y, color='red', label='Real yb')
            ...
            plt.legend()
            # plt.show()
            plt.savefig('./rfimage/' + t + '_' + n[condition] + '_' + th + '.png')
            plt.close()
            mse = mean_squared_error(test_Y, predicted_yb_count)
            mae = mean_absolute_error(test_Y, predicted_yb_count)
            rmse = sqrt(mse)
            mape = np.mean(np.abs((test_Y - predicted_yb_count) / test_Y)) * 100
