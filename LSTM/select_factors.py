...
def select_feature(filename, data, c):
    ...
    second_time_columns[0] = second_time_data
    third_time_columns[0] = third_time_data
...
for t in traindata:
    for th in Threshold_Folder:
        y_train_Age1 = readfile(os.path.join(workspace_dir, t + "/excess_" + th), True)
        ...
        first_time_columns, second_time_columns, third_time_columns = select_feature(y_train_Age1, workday, c)
        print(len(first_time_columns), len(second_time_columns), len(third_time_columns))
        time.sleep(100)
        # first_time_columns, second_time_columns, third_time_columns = select_feature(y_train_Age1, weekand, c)
        first_time_columns.to_csv('./data/select_data/' + t + '_' + th + '_' + str(select_hour[c][0]) + '.csv')
        second_time_columns.to_csv('./data/select_data/' + t + '_' + th + '_' + str(select_hour[c][1]) + '.csv')
        third_time_columns.to_csv('./data/select_data/' + t + '_' + th + '_' + str(select_hour[c][2]) + '.csv')
